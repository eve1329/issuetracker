...
- Optionally post one configurable reply back to an internal Issue after the first non-author comment is detected. External Issues remain notification-only; the first enablement creates a silent baseline instead of replaying historical comments.
...
- `Automatically reply once to internal Issues after their first response?`: opt-in write-back for internal Issue authors only. The first successful run after enabling establishes a silent baseline; failed replies remain pending for a later successful sync.
...
- When internal auto-reply is enabled, the first successful run silently records already-observed internal first responses. Later successful syncs post the configured reply once for each newly detected internal first response; failed write-backs remain pending. External Issues continue through the local/Feishu reminder path and are never auto-replied.
...
- 可选地在首次检测到内部 Issue 的非作者评论后自动回写一条配置好的回复；外部 Issue 仍以本机和飞书提醒为主，不自动回写。首次开启时会静默建立基线，不会重放历史评论；失败的自动回复会保留到后续成功同步重试。
...
- `内部 Issue 首次响应后自动回复一次？`：仅对内部 Issue 开启一次性写回；首次成功同步会静默建立已有首次响应基线。
...
- 开启内部自动回复后，第一次成功同步会静默记录已有的内部首次响应；后续成功同步在首次检测到新的内部非作者评论时只回复一次，失败会保留到待处理队列。外部 Issue 继续只走本机和飞书提醒，不会自动回写。
