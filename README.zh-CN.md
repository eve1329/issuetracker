...
- 对内部 Issue，可选地在创建满设定时长后，若仍为 open 且没有非作者评论，则自动回写一条回复；默认延迟为 24 小时。外部 Issue 不自动回写。
...
- `飞书群机器人 Webhook`：可选。插件只会直接从 Obsidian 向群机器人发送新增外部 Issue；每条成功投递都会记录，未成功投递会在后续成功同步时重试；留空则只在本机提醒
...
- 第一次成功同步只会静默记录当前 Issue 集合；之后的成功同步会提醒新增的内部和外部 Issue。飞书仅投递新增外部 Issue，成功后才从待投递队列移除，失败会在后续成功同步时重试。失败或降级同步不会推进通知基线。重叠的同步触发会复用同一次运行，避免重复进度条和对同一生成文件的竞争写入。
- 开启内部自动回复后，首次成功同步会静默建立基线；后续成功同步仅将新发现的内部、open、无非作者评论 Issue 进入待处理队列，在创建时间加上配置延迟到期后只回复一次，失败会保留到待处理队列。外部 Issue 不会自动回写。
